...
export function FeedbackClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitted(true);
      setTimeout(() => router.push(`/mentor/mentorias/${appointmentId}`), 2000);
    } catch (err) {
      ...
      <div className="container px-4 py-8">
        <div className="mx-auto max-w-2xl">
          <Card>
            ...
            <CardContent>
              <Button asChild>
                <Link href={`/mentor/mentorias/${appointmentId}`}>Voltar para mentoria</Link>
              </Button>
            </CardContent>
          </Card>
        </div>
  ...
  return (
    <div className="container px-4 py-8">
      <Button variant="ghost" asChild className="mb-4">
        <Link href={`/mentor/mentorias/${appointmentId}`}>
      ...
      <div className="mx-auto max-w-2xl">
        <div className="mb-6 text-center">
          ...
          <p className="mt-2 text-muted-foreground">
            Compartilhe sua experiência sobre a sessão
          </p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>Feedback do Mentor</CardTitle>
            <CardDescription>
              Sua avaliação ajuda a melhorar o sistema e acompanhar o progresso dos estudantes
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex items-center justify-between rounded-lg border p-4">
                <div className="space-y-0.5">
                  <Label htmlFor="feedback-public">Feedback público</Label>
                  <p className="text-sm text-muted-foreground">
                    Se ativado, sua avaliação poderá ser exibida publicamente (ex.: perfil do mentor).
                  </p>
                </div>
                <Switch
                  id="feedback-public"
                  ...
                  onCheckedChange={(checked) =>
                    setFeedback((prev) => ({ ...prev, isPublic: checked }))
                  }
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  id="comment"
                  placeholder="Compartilhe observações sobre a sessão, progresso do estudante, etc..."
